...
from quora_rest_api.quora_docs_service import get_query_result
from quora_rest_api.quora_indexing import get_query_relevant_docs, build_docs_index
from quora_rest_api.quora_lemmatize_text import get_lemmatize_texts
from quora_rest_api.quora_suggestions import get_suggestions, build_queries_index
...
def search():
    ...
    resp = requests.post("http://127.0.0.1:5000//lemmatizer", json=payload)
...
def lemmatizer():
    ...
    resp = requests.post("http://127.0.0.1:5000//indexing", json=payload)
    return resp.text, resp.status_code, resp.headers.items()
...
def indexing():
    ...
    resp = requests.post("http://127.0.0.1:5000//get_docs", json=payload)
    return resp.text, resp.status_code, resp.headers.items()
